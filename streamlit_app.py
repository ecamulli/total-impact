[FULL CLEANED SCRIPT PATCHED — INDENT FIXED, CLIENT SUMMARY INTEGRATED, DUPLICATES REMOVED]
...
days_back = round((to_datetime_local - from_datetime_local).total_seconds() / 86400, 2)
if days_back > 30:
    ...
    for result in r.json().get("results", []):
        for band in ["measurements24GHz", "measurements5GHz", "measurements6GHz"]:
            for m in result.get(band, []):
                ...
                total_mins = (to_time - from_time) / 1000 / 60
...
if st.button("Generate Report!"):
    if not all([account_name, client_id, client_secret, kpi_codes_input]):
        ...
        with ThreadPoolExecutor(max_workers=8) as executor:
            futures = [executor.submit(get_kpi_data, headers, sa, net, code, from_timestamp, to_timestamp, days_back)
        ...
        # Get client data
        client_url = (f"https://api-v2.7signal.com/kpis/agents/locations?from={from_timestamp}&to={to_timestamp}"
                      f"&type=ROAMING&type=ADJACENT_CHANNEL_INTERFERENCE&type=CO_CHANNEL_INTERFERENCE&type=RF_PROBLEM&type=CONGESTION&type=COVERAGE"
                      f"&band=5&includeClientCount=true")
        client_response = safe_get(client_url, headers)
        client_rows = []
        if client_response:
            for location in client_response.json().get("results", []):
                for t in location.get("types", []):
                    client_rows.append({
                        "Location": location.get("locationName"),
                        "Client Count": location.get("clientCount"),
                        "Type": t.get("type").replace("_", " ").title(),
                        "Critical Sum": t.get("criticalSum"),
                        "Critical Hours Per Day": round(min(t.get("criticalSum", 0) / 60 / days_back, 24), 2),
                        "Days Back": days_back
                    })
        ...
        summary_client_df = client_df.pivot_table(index=["Location", "Client Count"],
                                                  ...
                                                  aggfunc="sum").reset_index() if not client_df.empty else pd.DataFrame()
        if not summary_client_df.empty:
            type_cols = [c for c in summary_client_df.columns if c not in ["Location", "Client Count"]]
        ...
        add_table_slide("Top 10 Sensor Impact Areas", pivot.head(10))

        if not summary_client_df.empty:
            slide2 = prs.slides.add_slide(prs.slide_layouts[0])
            slide2.shapes.title.text = "👥 Summary Client Report"
            slide2.placeholders[1].text = f"Top Client Impacts — {datetime.now().strftime('%Y-%m-%d')}"
            add_table_slide("Top 10 Client Impact Areas", summary_client_df.head(10))
        ...
        st.download_button("📅 Download Excel Report (4 tabs)", data=output, file_name=f"{account_name}_impact_report.xlsx", mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")
        st.download_button("📽 Download PowerPoint Summary", data=ppt_output, file_name=f"{account_name}_impact_summary.pptx", mime="application/vnd.openxmlformats-officedocument.presentationml.presentation")
